...
class NamedBlocksPolyfill {
  ...
  ExitBlock(node) {
    ...
    assert(
      ...
      () =>
        `Invalid nesting: expecting block ${JSON.stringify(
          node
  ...
  ExitProgram(node) {
    if (node.type === 'Template') {
      ...
      assert(
        node === actual,
        `Invalid nesting: expecting template ${JSON.stringify(
          node
        )} got ${JSON.stringify(actual)}`
      );
  ...
  namedBlocksFor(node) {
    ...
    for (let statement of node.children) {
      if (
        isComment(statement) ||
        (isTextNode(statement) && isWhitespace(statement))
      ) {
        continue;
      ...
      b.mustache(
        ...
        b.hash(
          namedBlocks.map((block) =>
            b.pair(block.tag.slice(1), b.number(block.blockParams.length))
          )
        )
      )
    );
  ...
  transformNamedBlock({ tag, children, blockParams, loc }) {
    ...
    let params = [
      b.sexpr(b.path('-is-named-block-invocation'), [
        b.path('__arg0'),
        b.string(name),
      ]),
    ];
  ...
  transformHasBlock(node, hasBlockParams = false) {
    ...
    if (block === 'default') {
      ...
      // Avoid visiting this node again and trigger an infinite loop
      fallback = this.skip(
        b.sexpr(path, [b.string('inverse')], undefined, loc)
      );
    } else {
    ...
    path = hasBlockParams
      ? b.path('-has-block-params', path.loc)
      : b.path('-has-block', path.loc);
  ...
  get builders() {
     ...
     */
    let blockItself = function buildBlockItself(
      body = [],
      blockParams = [],
      _chained,
      loc
    ...
    if (element.length === 2) {
       ...
       * @param {Partial<NewElementBuilderOptions>} options
       * @returns {ElementNode}
       *
       ...
       * @returns {ElementNode}
       *
       * @typedef {Parameters<CanonicalElementBuilder>} CanonicalElementBuilderArgs
       *
       ...
       * @returns {ElementNode}
       *
       * @typedef {Parameters<LegacyElementBuilder>} LegacyElementBuilderArgs
       *
       * @typedef {CanonicalElementBuilderArgs | LegacyElementBuilderArgs} ElementBuilderArgs
       *
       * @param {ElementBuilderArgs} args
       * @returns {args is CanonicalElementBuilderArgs}
       */
      ...
      let buildElement = function buildElement(...args) {
        ...
        /**
         * @type {Partial<NewElementBuilderOptions>}
         */
        let options;
