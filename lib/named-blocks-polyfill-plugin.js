...
class NamedBlocksPolyfill {
  ...
  namedBlocksFor(node) {
    ...
    for (let statement of node.children) {
      if (isComment(statement) || isTextNode(statement) && isWhitespace(statement)) {
        ...
        if (name === 'main') {
          check(
            names.indexOf('default') === -1,
            `Cannot pass named blocks <:main> and <:default> in the same invocation`,
            statement
          );

          name = 'default';
          statement.tag = ':default';
        }
  ...
  transformNamedBlocks(node, namedBlocks) {
    ...
    if (namedBlocks.length === 1 && namedBlocks[0].tag === ':default' || namedBlocks[0].tag === ':main') {
      return b.element(
        node.tag,
        node.attributes,
        node.modifiers,
  ...
  transformYield(node) {
    ...
    if (to.value === 'default' || to.value === 'main') {
      hash = null;
